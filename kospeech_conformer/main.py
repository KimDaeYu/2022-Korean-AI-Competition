...
from kospeech.models.conformer import *
...
if __name__ == '__main__':
    ...
    #모델 파람
    args.add_argument('--architecture', type=str, default='conformer') # 수정
    # small size params
    args.add_argument('--input_size',type=int,default=80)
    args.add_argument('--encoder_dim', type=int, default=144)
    args.add_argument('--decoder_dim', type=int, default=320)
    ...
    args.add_argument('--transform_method', type=str, default='fbank') #이게  fbank로 인식안되고 spect로 들어가졌음.
    ...
    print(config)
    #config.audio = FilterBankConfig() # 오류원인 여기.
    #kospeech는 config.audio 이렇게 다 분리됐는데, 여기서는 따로 분류가 안되서 생긴 문제.
    #이렇게 되면 앞의 argument들은 사용이 안된다.

    # #Conformer
    #config.model = ConformerSmallConfig()  # ConformerLargeConfig
    #config.train = ConformerSmallTrainConfig()
